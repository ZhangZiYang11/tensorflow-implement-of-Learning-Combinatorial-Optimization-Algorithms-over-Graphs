...
class ReplayBuffer:
    ...
    def __init__(self, size, obs_size, n_nodes):#n_nodes应该是max值
        self.size = size
        self.obs = np.zeros([self.size, obs_size, 2], dtype=np.float32)#生成2维的np列表，size实际为1000000，obs_size为每个样本的obs维数
        ...
        self.next_obs = np.zeros([self.size, obs_size, 2], dtype=np.float32)
        ...
        self.edge_feat = np.zeros([self.size, n_nodes, n_nodes, 4], dtype=np.float32)
        self.aux = np.zeros([self.size, n_nodes, 4],dtype=np.float32)
        self.e2n_param = np.zeros([self.size, n_nodes],dtype=np.float32)

        self.num_in_buffer = 0
        self.next_idx = 0


    def store_transition(self, obs, adj, weight_matrix, action, n_q_value, n_act, reward, next_obs, done, transition_length, true_node, adjust_matrix, edge_feat, aux, e2n_param):
        ...
        self.edge_feat[self.next_idx] = edge_feat
        self.aux[self.next_idx] = aux
        self.e2n_param[self.next_idx] = e2n_param
    ...
    def sample(self, batch_size):
        ...
        #论文中使用了n-step Q-learning，所以reward应该是n步的所有reward总和，next_obs应该是n步之后的状态，可以通过输入数据时实现
        return self.obs[idxes], \
               ...
               self.edge_feat[idxes],\
               self.aux[idxes],\
               self.e2n_param[idxes]
