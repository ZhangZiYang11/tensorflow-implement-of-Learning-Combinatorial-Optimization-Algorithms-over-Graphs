...
class ReplayBuffer:
    ...
    def __init__(self, size, obs_size, n_nodes):#n_nodes应该是max值
        self.size = size
        self.obs = np.zeros([self.size, obs_size, 4], dtype=np.float32)#生成2维的np列表，size实际为1000000，obs_size为每个样本的obs维数
        ...
        self.next_obs = np.zeros([self.size, obs_size, 4], dtype=np.float32)
    ...
    def store_transition(self, obs, adj, weight_matrix, action, n_q_value, n_act, reward, next_obs, done, transition_length, true_node, adjust_matrix, edge_feat):
