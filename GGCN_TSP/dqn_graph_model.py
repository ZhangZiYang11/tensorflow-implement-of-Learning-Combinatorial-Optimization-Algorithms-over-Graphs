...
def learn(env,
    ...
    test_env = tsp_env_random.tsp_env_random(num_min,num_actions)
    ...
    obs_t_ph              = tf.placeholder(tf.float32, [None] + list(input_shape)+[4])#占位符，tf.placeholer(dtype, shape=None, name=None),shape为[None]表示形状不指定
    ...
    obs_tp1_ph            = tf.placeholder(tf.float32, [None] + list(input_shape)+[4])
    ...
    real_nodes, now_state, node_feat_choose, adj_choose, w_choose, adjust_choose, edge_feat_choose = env.reset()
    ...
    for t in itertools.count():#从0开始步长为1的无限序列
        ...
        if done:
            if one_graph_test != True:
                ...
                real_nodes, now_state, node_feat_choose, adj_choose, w_choose, adjust_choose, edge_feat_choose = env.one_graph_reset()
        ...
        q_values=session.run(q_func_net, feed_dict={obs_t_ph: node_feat_choose[None],
                                                    ...
                                                    edge_feat_ph:edge_feat_choose[None]
                                                    })
        ...
        now_state, next_node_feat_choose, step_reward, done, adj_choose, w_choose, adjust_choose, next_edge_feat_choose = env.step(action)
        
        eval_n_q_values=session.run(target_q_func_net, feed_dict={obs_tp1_ph: next_node_feat_choose[None],
                                                        adj_ph: adj_choose[None],
                                                        graph_weights_ph: w_choose[None],adjust_matrix_ph:adjust_choose[None],multiply_matrix_ph:multiply_choose[None],
                                                        edge_feat_ph:next_edge_feat_choose[None]})
        ...
        if (t > learning_starts and
            ...
            rew_batch, obs_tp1_batch, done_mask_batch, transition_length_batch, true_nodes_batch, adjust_batch, edge_feat_batch= replay_buffer.sample(batch_size)
            ...
            if t % 1000 == 0 :
                ...
                if one_graph_test != True:
                    for test_ep in range(100):
                        test_done=False
                        test_real_nodes, test_state, test_x, test_adj, test_w, test_adjust, test_edge_feat = test_env.reset()
                        for test_step in range(num_actions + 10):
                            if test_done:
                                ...
                                model_rew = test_env.distance(test_env.ordered_action)
                                opt_rew, opt_solution= test_env.optimal_solution()
                            ...
                            test_q_values=session.run(q_func_net, feed_dict={obs_t_ph: test_x[None],
                                                                    ...
                                                                    edge_feat_ph: test_edge_feat[None]})
                            test_action = np.argmax((test_q_values[0] * (1 - test_state) - 1e5 * test_state))
                            test_state, test_x, test_reward, test_done, test_adj, test_w, test_adjust, test_edge_feat= test_env.step(test_action)
